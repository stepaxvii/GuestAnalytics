...
def month_insight(reviews_block, count_insights):
    "Определение семантического настроения отдельного отзыва."

    openai.api_key = OPENAI_API_KEY

    prompt = (
        f"""Ты — профессиональный аналитик ресторанного бизнеса
        и эксперт по обработке клиентских отзывов.
        Тебе дан массив отзывов гостей.
        Твоя задача — выявить {count_insights} ключевых инсайтов,
        ...
        - НЕ используй общие фразы типа "вкусная барабулька"
        или "уютная атмосфера".
        - НЕ дублируй одно и то же разными словами.
        - НЕ выдавай инсайты,
        если их нет — в этом случае просто напиши "Нет значимых инсайтов".

        Формат ответа:
        - Только четкие, осмысленные инсайты без вводных слов.
        - Каждый инсайт — отдельной строкой, без номеров и маркеров.
        - Отвечай строго по делу, без пояснений,
        JSON-структур и лишнего текста.
